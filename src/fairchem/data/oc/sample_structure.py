...
def parse_args():
    ...
    # input and output
    parser.add_argument(
        "--bulk_db", type=str, required=True, help="Underlying db for bulks"
    )
    parser.add_argument(
        "--adsorbate_db", type=str, required=True, help="Underlying db for adsorbates"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Input whether placement method is heuristic or random",
    )
    parser.add_argument(
        "--random_sites",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma separated list of bulk indices",
    )
    parser.add_argument(
        "--surface_index", type=int, default=None, help="Optional surface index (int)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Do not write out POTCAR/INCAR/KPOINTS",
    )
    ...
    if args.enumerate_all_structures:
        if args.adsorbate_index is None or args.bulk_indices is None:
            parser.error(
                "Enumerating all structures requires specified adsorbate and bulks"
            )
