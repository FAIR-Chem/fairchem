...
class StructureSampler:
    ...
    def __init__(self, args):
        ...
        if self.args.enumerate_all_structures:
            ...
            self.logger.info(
                f"Enumerating all surfaces/configs for adsorbate {self.args.adsorbate_index} and bulks {self.bulk_indices_list}"
            )
        else:
    ...
    def run(self):
        ...
        if self.args.enumerate_all_structures:
            self.adsorbate = Adsorbate(
                self.args.adsorbate_db, self.args.adsorbate_index
            )
        self._load_bulks()
    ...
    def _load_bulks(self):
        ...
        if self.args.enumerate_all_structures:
            for ind in self.bulk_indices_list:
                self.all_bulks.append(
                    Bulk(bulk_db_lookup, self.args.precomputed_structures, ind)
                )
        else:
            self.all_bulks.append(
                Bulk(bulk_db_lookup, self.args.precomputed_structures)
            )
        ...
        for bulk_ind, bulk in enumerate(self.all_bulks):
            ...
            if self.args.enumerate_all_structures:
                if self.args.surface_index is not None:
                    assert (
                        0 <= self.args.surface_index < len(possible_surfaces)
                    ), "Invalid surface index provided"
                    self.logger.info(
                        f"Loading only surface {self.args.surface_index} for bulk {self.bulk_indices_list[bulk_ind]}"
                    )
                    included_surface_indices = [self.args.surface_index]
                else:
                    self.logger.info(
                        f"Enumerating all {len(possible_surfaces)} surfaces for bulk {self.bulk_indices_list[bulk_ind]}"
                    )
                ...
                for cur_surface_ind in included_surface_indices:
                    ...
                    surface = Surface(
                        bulk, surface_info, cur_surface_ind, len(possible_surfaces)
                    )
                    self._combine_and_write(
                        surface, self.bulk_indices_list[bulk_ind], cur_surface_ind
                    )
            else:
                surface_info_index = np.random.choice(len(possible_surfaces))
                surface = Surface(
                    bulk,
                    possible_surfaces[surface_info_index],
                    surface_info_index,
    ...
    def _combine_and_write(self, surface, cur_bulk_index=None, cur_surface_index=None):
        ...
        if self.args.enumerate_all_structures:
            output_name_template = (
                f"{self.args.adsorbate_index}_{cur_bulk_index}_{cur_surface_index}"
            )
        else:
        ...
        if self.args.random_placements:
            ...
            combined = Combined(
                self.adsorbate, surface, self.args.enumerate_all_structures
            )
    ...
    def _write_surface(self, surface, output_name_template):
        ...
        self.logger.info(
            f"wrote surface ({bulk_dict['bulk_samplingstr']}) to {bulk_dir}"
        )
        ...
        for config_ind in range(combined.num_configs):
            if self.args.enumerate_all_structures:
                adslab_id = (
                    f"rand{config_ind}"
                    if self.args.random_placements
                    else f"heur{config_ind}"
                )
                adsorbed_bulk_dir = os.path.join(
                    self.args.output_dir, output_name_template, adslab_id
                )
            else:
                adsorbed_bulk_dir = os.path.join(
                    self.args.output_dir, output_name_template, "adslab"
                )
