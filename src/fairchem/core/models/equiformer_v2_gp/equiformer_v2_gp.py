...
from fairchem.core.models.equiformer_v2.layer_norm import (
...
@registry.register_model("equiformer_v2_gp")
class EquiformerV2_GP(BaseModel):
    """
    ...
    def forward(self, data):
        ...
        ### Graph Parallel: Partition edges
        node_partition = gp_utils.scatter_to_model_parallel_region(
            torch.arange(len(data.atomic_numbers)).to(self.device)
        )
        ...
        ### Graph Parallel: Partition nodes
        atomic_numbers_full = atomic_numbers
        atomic_numbers = atomic_numbers[node_partition]
        batch_full = data.batch
        data.batch = data.batch[node_partition]
        ...
        edge_degree = self.edge_degree_embedding(
            atomic_numbers, atomic_numbers_full, edge_distance, edge_index, node_offset
        )
        ...
        for i in range(self.num_layers):
            ...
            if self.checkpoint and self.training:
                x_embed = torch.utils.checkpoint.checkpoint(
                    ...
                    edge_distance,
                    edge_index,
                    data.batch,  # for GraphDropPath
                    node_offset,
                    use_reentrant=False,
                )
            else:
                x_embed = self.blocks[i](
                    x.embedding,
                    x,  # SO3_Embedding
                    atomic_numbers_full,
                    edge_distance,
        ...
        # Graph Parallel: Gather node energies
        ...
        energy.index_add_(0, batch_full, node_energy.view(-1))
