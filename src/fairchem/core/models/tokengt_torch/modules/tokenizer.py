...
class EdgeEncoder(nn.Module):
    def __init__(self, num_elements, hidden_dim):
        super().__init__()
        self.src_encoder = nn.Embedding(num_elements, hidden_dim, padding_idx=0)
        self.dst_encoder = nn.Embedding(num_elements, hidden_dim, padding_idx=0)
        self.proj = nn.Linear(2 * hidden_dim, hidden_dim)
    def forward(self, src, dst):
        return self.proj(torch.cat([self.src_encoder(src), self.dst_encoder(dst)], dim = -1))
...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.edge_encoder = EdgeEncoder(num_elements=num_elements, hidden_dim=hidden_dim)
        self.atom_pos_encoder = nn.Linear(3, hidden_dim)
        self.edge_pos_encoder = nn.Linear(3, hidden_dim)
        self.graph_token = nn.Parameter(torch.randn(1, hidden_dim))
        self.null_token = nn.Parameter(torch.randn(1, hidden_dim))  # this is optional
        ...
        if self.lap_node_id:
            self.lap_encoder = nn.Linear(2 * lap_node_id_dim, hidden_dim, bias=False)
    ...
    def add_special_tokens(self, padded_feature, padding_mask):
        ...
        graph_token_feature = self.graph_token.expand(b, 1, d)  # [1, D]
        null_token_feature = self.null_token.expand(b, 1, d)  # [1, D], this is optional
    ...
    def forward(
        ...
        # use r hat / r to reflect physics inductive bias
        edge_distance_vec = edge_distance_vec / edge_distance[:, None].square()

        node_feature = self.atom_encoder(atomic_numbers) + self.atom_pos_encoder(pos)
        edge_feature = (
            self.edge_encoder(atomic_numbers[edge_index[0]], atomic_numbers[edge_index[1]])
            + self.edge_pos_encoder(edge_distance_vec)
        )
        ...
        if self.lap_node_id:
            lap_dim = lap_vec.size(-1)
            if self.lap_node_id_dim > lap_dim:
                eigvec = F.pad(lap_vec, (0, self.lap_node_id_dim - lap_dim), value=float('0'))  # [sum(n_node), Dl]
            else:
                eigvec = lap_vec[:, :self.lap_node_id_dim]  # [sum(n_node), Dl]
        ...
        return padded_feature, padding_mask, padded_node_mask  # [B, 2+T, D], [B, 2+T], [B, T, 2], [B, T]
