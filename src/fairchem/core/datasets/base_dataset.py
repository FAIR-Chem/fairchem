...
class ConcatDataset(ConcatDataset_):
    """A pytorch ConcatDataset that also takes metadata if given."""
    ...
    def metadata_hasattr(self, attr) -> bool:
        return all(dataset.metadata_hasattr(attr) for dataset in self.datasets)
    ...
    def get_metadata(self, attr, idx):
        ...
        return self.datasets[dataset_idx].get_metadata(attr, sample_idx)
