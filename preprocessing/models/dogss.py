...
#         )        
        ...
        self.conv_to_bond_distance = nn.Linear(2*atom_embedding_size + bond_feat_dim, dist_feat_dim)
        self.bond_distance_bn = nn.BatchNorm1d(dist_feat_dim)

        self.conv_to_bond_constant = nn.Linear(2*atom_embedding_size + bond_feat_dim, const_feat_dim)
    ...
    def forward(self, data):
        ...
        if hasattr(self, 'layers_dist'):
            ...
            bond_distance = self.softplus((self.bond_distance(bond_distance)+distance))
        else:
            bond_distance = self.softplus(self.bond_distance_bn(bond_distance) + distance)
        ...
        if hasattr(self, 'layers_const'):
            ...
            bond_constant = torch.mean(bond_constant, dim=1).unsqueeze(-1) / len(bond_constant)
        ...
        while (torch.max(torch.abs(steepest_descent_step))>0.0005 and step_count<self.max_opt_steps) or step_count<self.min_opt_steps:
            ...
            distance = self.get_distance(atom_pos, cells, edge_index, nbr_fea_offset)
...
#         print('out',atom_pos[free_atom_idx].shape)
    ...
    def get_distance(self, atom_pos, cells, edge_index, nbr_fea_offset):
        differ = atom_pos[edge_index[1]] - atom_pos[edge_index[0]] + torch.bmm(nbr_fea_offset.view(-1,self.max_num_nbr,3), cells).view(-1,3)
