...
class GemNet(ScaledModule):
    ...
    def get_triplets(self, graph, num_atoms):
        """
    ...
    def get_mixed_triplets(
        ...
        if to_outedge:
            ...
            cell_offsets_sum = (
                graph_out["cell_offset"][idx_out]
                + graph_in["cell_offset"][idx_in]
            )
        else:
            idx_atom_in = idx_in_s[idx_in]
    ...
    def get_quadruplets(
        ...
        idx["triplet_in"] = self.get_mixed_triplets(
            main_graph,
            qint_graph,
            num_atoms,
            to_outedge=True,
        ...
        idx["triplet_out"] = self.get_mixed_triplets(
            qint_graph,
            main_graph,
            num_atoms,
            to_outedge=False,
        ...
        num_trip_out_per_inter = num_trip_in_per_inter[
            idx["triplet_out"]["in"]
        ]
    ...
    def get_graphs_and_indices(self, data):
        ...
        trip_idx_e2e = self.get_triplets(main_graph, num_atoms=num_atoms)

        # Additional indices for quadruplets
        if self.quad_interaction:
            quad_idx = self.get_quadruplets(
                main_graph,
                qint_graph,
                num_atoms,
            )
        ...
        if self.atom_edge_interaction:
            trip_idx_a2e = self.get_mixed_triplets(
                a2ee2a_graph,
                main_graph,
                num_atoms=num_atoms,
                return_agg_idx=True,
            ...
            trip_idx_a2e = {}
        if self.edge_atom_interaction:
            trip_idx_e2a = self.get_mixed_triplets(
                main_graph,
                a2ee2a_graph,
                num_atoms=num_atoms,
                return_agg_idx=True,
