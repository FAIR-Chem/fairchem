...
RNGStates = Tuple[
    ByteTensor, Optional[ByteTensor]
]  # (cpu_rng_state, gpu_rng_state)
...
class Recompute(torch.autograd.Function):
    ...
    def backward(
        ...
        input_leaf = tuple(
            x.detach().requires_grad_(x.requires_grad) for x in input
        )

        with restore_rng_states(input[0].device, ctx.rng_states):
            with torch.enable_grad(), enable_recomputing():
                output = ctx.function(
                    input_leaf[0] if ctx.input_atomic else input_leaf
                )
