...
class Rank2Block(nn.Module):
    ...
    def forward(self, edge_distance_vec, x_edge, edge_index, data):
        ...
        outer_product_edge = torch.bmm(edge_distance_vec.unsqueeze(2), edge_distance_vec.unsqueeze(1))
        ...
        if self.edge_level:
            ...
            node_outer = scatter(edge_outer, edge_index, dim=0, reduce="mean") 
            if self.mixing_coordinates:
                node_outer = node_outer.view(len(data.fixed), -1) # (natoms, 9 * emb_size_edge) 
                for module in self.stress_MLP:
                    node_outer =  module(node_outer) # (nAtoms,  num_targets) # stress_MLP : emb -> num_targets (9)
            else:
