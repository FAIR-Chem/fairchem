...
def _basis_transformation_Q_J(
    ...
    with torch_default_dtype(torch.float64):
        ...
        def _R_tensor(a, b, c):
            return kron(
                irr_repr(order_out, a, b, c), irr_repr(order_in, a, b, c)
            )
        ...
        assert (
            null_space.size(0) == 1
        ), null_space.size()  # unique subspace solution
        ...
        assert all(
            torch.allclose(
                _R_tensor(a, b, c) @ Q_J, Q_J @ irr_repr(J, a, b, c)
            )
...
def get_spherical_from_cartesian_torch(cartesian, divide_radius_by=1.0):
    ...
    if divide_radius_by == 1.0:
        spherical[..., ind_radius] = torch.sqrt(
            r_xy + cartesian[..., cartesian_z] ** 2
        )
    else:
        spherical[..., ind_radius] = (
            torch.sqrt(r_xy + cartesian[..., cartesian_z] ** 2)
            / divide_radius_by
        )
...
def get_spherical_from_cartesian(cartesian):
    ...
    # get overall radius
    # ptsnew[:,3] = np.sqrt(xy + xyz[:,2]**2)
    spherical[..., ind_radius] = np.sqrt(
        r_xy + cartesian[..., cartesian_z] ** 2
    )

    # get second angle
    # version 'elevation angle defined from Z-axis down'
    spherical[..., ind_beta] = np.arctan2(
        np.sqrt(r_xy), cartesian[..., cartesian_z]
    )
    # ptsnew[:,4] = np.arctan2(np.sqrt(xy), xyz[:,2])
    # version 'elevation angle defined from XY-plane up'
    # ptsnew[:,4] = np.arctan2(xyz[:,2], np.sqrt(xy))
