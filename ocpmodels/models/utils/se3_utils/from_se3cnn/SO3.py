...
def irr_repr(order, alpha, beta, gamma, dtype=None):
    ...
    return torch.tensor(
        wigner_D_matrix(
            order, np.array(alpha), np.array(beta), np.array(gamma)
        ),
...
def xyz_vector_basis_to_spherical_basis():
    ...
    with torch_default_dtype(torch.float64):
        A = torch.tensor(
            [[0, 1, 0], [0, 0, 1], [1, 0, 0]], dtype=torch.float64
        )
        assert all(
...
def tensor3x3_repr_basis_to_spherical_basis():
    ...
    with torch_default_dtype(torch.float64):
        ...
        assert all(
            torch.allclose(
                irr_repr(0, a, b, c) @ to1, to1 @ tensor3x3_repr(a, b, c)
            )
            for a, b, c in torch.rand(10, 3)
        )
        ...
        assert all(
            torch.allclose(
                irr_repr(1, a, b, c) @ to3, to3 @ tensor3x3_repr(a, b, c)
            )
            for a, b, c in torch.rand(10, 3)
        )
        ...
        assert all(
            torch.allclose(
                irr_repr(2, a, b, c) @ to5, to5 @ tensor3x3_repr(a, b, c)
            )
            for a, b, c in torch.rand(10, 3)
        )
...
def _test_change_basis_wigner_to_rot():
    ...
    with torch_default_dtype(torch.float64):
        ...
        r1 = (
            A.t()
            @ torch.tensor(wigner_D_matrix(1, a, b, c), dtype=torch.float64)
            @ A
        )
        r2 = rot(a, b, c)
