...
class OCPTrainer(BaseTrainer):
    ...
    def load_task(self):
        ...
        for target in self.targets:
            if "irreps" in self.targets[target]:
                self.train_targets[target] = self.targets[target]
            else:
                for subtarget in self.targets[target]:
        ...
        self.eval_metrics = self.config["task"]["metrics"]

        # assert len(self.targets.keys() - self.eval_metrics.keys()) == 0
