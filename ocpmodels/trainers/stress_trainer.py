...
@registry.register_trainer("stress")
class StressTrainer(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force & Stress (S2EFS) and Initial State to
    ...
    def run_relaxations(self, split="val"):
        ...
        for i, batch in tqdm(
            ...
            relaxed_batch = ml_relax(
                batch=batch,
                model=self,
                steps=self.config["task"].get("relaxation_steps", 100),
                fmax=self.config["task"].get("relaxation_fmax", 0.0),
                relax_opt=self.config["task"]["relax_opt"],
                stress=self.config["task"].get("relax_stress", True),
                isotropic=self.config["task"].get("isotropic", False),
                opt_forces=self.config["task"].get("opt_forces", True),
                opt_stress=self.config["task"].get("opt_stress", True),
                cell_factor=self.config["task"].get("cell_factor", 0.1),
                device=self.device,
                transform=None,
            )
