...
class Evaluator:
    ...
    task_attributes = {
        ...
        "is2re": ["energy"],
        "s2efs": ["energy", "forces", "stress_isotropic", "stress_anisotropic", "natoms"],
    ...
    def __init__(self, task=None):
        assert task in ["s2ef", "is2rs", "is2re", "s2efs", "s2es", "s2e", "s2s"]
        self.task = task
...
def stress_mae(prediction, target):
    ...
    change_mat = torch.tensor([[3 **(-0.5), 0, 0, 0, 3 **(-0.5), 0, 0, 0, 3 **(-0.5)], 
                 ...
                 ]).detach().to(prediction["stress_isotropic"].device)
    zero_vectors = torch.zeros((prediction["stress_isotropic"].shape[0], 3), device=prediction["stress_isotropic"].device)
    prediction_irreps = torch.concat([prediction["stress_isotropic"].reshape(-1, 1), 
                                      zero_vectors, 
                                      prediction["stress_anisotropic"].reshape(-1, 5)], dim=1)
    prediction_stress = torch.einsum("ba, cb->ca", change_mat, prediction_irreps)
    
    target_irreps = torch.concat([target["stress_isotropic"].reshape(-1, 1), 
                                      zero_vectors, 
                                      target["stress_anisotropic"].reshape(-1, 5)], dim=1)
    target_stress = torch.einsum("ba, cb->ca", change_mat, target_irreps)

    return absolute_error(prediction_stress.reshape(-1) * 0.1,
                          target_stress.reshape(-1) * 0.1) \
                              
def stress_isotropic_mae(prediction, target):
    # Multiplying by 0.1 to convert the stress to GPa as in M3GNet
    return absolute_error(prediction["stress_isotropic"] * 0.1, target["stress_isotropic"] * 0.1) 

def stress_anisotropic_mae(prediction, target):    
    # Multiplying by 0.1 to convert the stress to GPa as in M3GNet
    return absolute_error(prediction["stress_anisotropic"] * 0.1, target["stress_anisotropic"] * 0.1) 
