...
def compare(args):
    ...
    for sysid in sysids:
        ...
        if not np.max(np.abs(delta)) < 1e-6:
            print(f"First frame of {sysid} trajectory doesn't match its atoms object input")
...
def create_parser():
    ...
    parser.add_argument("--sysid_file", type=str, help="A txt file constains all the system ids (random ids) of the dataset")
    ...
    parser.add_argument("--input_dir_by_sysid", type=str, help="A pickle file that contains a dictionary that maps input folder path, which has metadata.pkl, to system ids")
    parser.add_argument("--num_workers",  type=int, help="Number of batch to split the inputs to preprocess")
    return parser
