# modified on predict.py
# 1. use modify_data instead
# 2. modified test_result.csv name
...
def predict_cgcnn(args):
    ...
    validate(args, model_args, test_loader, model, criterion, normalizer, test=True)


def validate(args, model_args, val_loader, model, criterion, normalizer, test=False):
    ...
    if test:
        star_label = '**'
        import csv
        with open('test_results_predict.csv', 'w') as f:
            writer = csv.writer(f)
